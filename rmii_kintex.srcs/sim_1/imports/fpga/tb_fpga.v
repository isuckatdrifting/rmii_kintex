...
always @(posedge eth_ref_clk) begin
  if(eth_rxdv_en) begin
    ...
    eth_crs_dv <= #`DFF 1;
    if(!arp_send) begin
      eth_rxd <= #`DFF arp[cfg_counter][sft[sft_cnt]+:2];
      if(sft_cnt == 15) begin
...
initial begin
  ...
  cfg[8]  = {32'ha8c07e60}; // ip.checksum(2B), ip.src(4B)                        0x1018
  ...
  cfg[14] = {1'b1, 7'h01, {8{1'b0}}, 16'd1000}; // pulse count                     0x1030
  //                      pw, freq_div
  ...
  //                          hist_mode_flag
  cfg[17] = {1'b1, 7'h04, 8'h4b, 8'h00, 4'ha, 2'b00, 2'b01}; //                                  0x103c
  //               hist_c2, hist_c1
  cfg[18] = {1'b1, 7'h05, 12'd250, 12'd250}; //                                     0x1040
  //               hist_c4, hist_c3
  cfg[19] = {1'b1, 7'h06, 12'd250, 12'd250}; //                                     0x1044
  //               hist_upth, hist_loth
  cfg[20] = {1'b1, 7'h07, 12'hFFF, 12'h000}; //                                   0x1048
  //                          cfg_ready
  cfg[21] = {1'b1, 7'h08, 8'h0,8'h4, 8'b1}; //                                   0x104c
  //                       1: oneshot/0: cont, system start
  cfg[22] = {1'b1, 7'h09, {22{1'b0}}, 1'b0, 1'b1}; //                             0x1050
  // User config ends
  cfg[23] = {32'h0ab429ba}; // fcs
  cfg_counter = 8'h00;
  sft_cnt = 0;
end
initial begin
  arp[0] = {32'h55555555};
  arp[1] = {32'hd5555555};
  arp[2] = {32'hffffffff};//{32'h005e0000};
  arp[3] = {32'h76f0ffff};//{32'h76f0cefa};
  arp[4] = {32'h2e4da91c};
  arp[5] = {32'h01000608};
  arp[6] = {32'h04060008};
  arp[7] = {32'h76f00100};
